...
OUTPUT_ARCH(mep)
ENTRY(_start)
SEARCH_DIR("=//mep-elf/lib"); SEARCH_DIR("=/usr/local/lib"); SEARCH_DIR("=/lib"); SEARCH_DIR("=/usr/lib");
/* Do we need any of these for elf?
   __DYNAMIC = 0;    */
SECTIONS
{
  /* Read-only sections, merged into text segment: */
  /* VENEZIA Inject: change location counter according to your base inject address, default:  0x8E3790 */
  PROVIDE (__executable_start = 0x8E3790); . = 0x8E3790;
  .interp         : { *(.interp) }
  .hash           : { *(.hash) }
  .dynsym         : { *(.dynsym) }
  .dynstr         : { *(.dynstr) }
